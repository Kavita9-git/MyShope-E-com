...
import { getCart, clearCart } from "../redux/features/auth/cartActions";
import {
  createOrder,
  processPayment,
  ...
  const saveCurrentAddressAsLastUsed = async () => {
    try {
      if (name && address && city && country && phone) {
        ...
        console.log(
          "Saved shipping details as last used address from checkout"
        );
      }
    } catch (error) {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (paymentMethod === "COD") {
      ...
      try {
        ...
        Alert.alert(
          "Error",
          "Something went wrong with payment processing. Please try again."
        );
      }
    }
  };
